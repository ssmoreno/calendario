...
A PDF response decoded as raw bytes, replacement characters, or other binary-looking text is not readable content. Do not summarize it. Search for the exact PDF URL and its identifiers, then use web_fetch on a clearly matching HTML abstract, official landing page, transcript, or accessible full-text copy found in the results. Prefer the publisher, author, institution, or canonical repository. Use a third-party copy only when its identity is unambiguous.
